...
class Problem:
    ...
    def __init__(self, par ,**kwargs):
        ...
        if "leave_coeff" in event.keys():
            self.leave_coeff = event['leave_coeff']
        else:
            self.leave_coeff = 0.005

        if "utility_intercept" in event.keys():
            self.utility_intercept = event['utility_intercept']
        else:
            self.utility_intercept = -1

        if "REG_delta_x_coeff" in event.keys():
            self.REG_delta_x_coeff = event['REG_delta_x_coeff']
        else:
            self.REG_delta_x_coeff = 0.0092

        if "REG_y_intercept" in event.keys():
            self.REG_y_intercept = event['REG_y_intercept']
        else:
            self.REG_y_intercept = 0.341

        if "SCH_delta_x_coeff" in event.keys():
            self.SCH_delta_x_coeff = event['SCH_delta_x_coeff']
        else:
            self.SCH_delta_x_coeff = 0.0092

        if "SCH_y_intercept" in event.keys():
            self.SCH_y_intercept = event['SCH_y_intercept']
        else:
            self.SCH_y_intercept = 0
        ...
        if self.par.sim == True:
            self.dcm_charging_flex_params = np.array([[ - self.power_rate * self.SCH_delta_x_coeff], [ self.power_rate * self.SCH_delta_x_coeff], [0], [self.SCH_y_intercept]])
            #% DCM parameters for choice 1 -- charging with flexibility
            self.dcm_charging_asap_params = np.array([[self.power_rate * self.REG_delta_x_coeff], [- self.power_rate * self.REG_delta_x_coeff], [0],[self.REG_y_intercept  ]])
            #% DCM parameters for choice 2 -- charging as soon as possible
            self.dcm_leaving_params = np.array([[self.power_rate * self.leave_coeff / 2], [self.power_rate * self.leave_coeff / 2], [0], [self.utility_intercept]])

        if self.par.sim == False:
            ...
            #% DCM parameters for choice 2 -- charging as soon as possible
            self.dcm_leaving_params = np.array([[self.power_rate * self.leave_coeff / 2], [self.power_rate * self.leave_coeff / 2], [0], [-1 ]])
...
class Optimization:
    ...
    def argmin_v(self, u, z):
        ...
        #             return softmax(THETA @ z).reshape(3,1)

        if sum(v.value) < 0 | (np.sum(v.value) < 1 - self.Parameters.soft_v_eta) | (np.sum(v.value) > 1 + self.Parameters.soft_v_eta):
            print('[ ERROR] invalid $v$')
            return softmax(THETA @ z).reshape(3,1)
...
#         vehicle_power_rate = self.Problem.power_rate
        ...
        if prob.status != "optimal":
            ...
            print(self.Problem.user_time, self.Problem.power_rate)
            # prob.solve(solver ='SCS') 
            # print(  "SCS, z status",prob.status)
    ...
    def run_opt(self):
        ...
        opt["asap_powers"] = asap_powers.flatten()
        opt["v"] = vk
        opt["prob_flex"] = vk[0]
        opt["prob_asap"] = vk[1]
        opt["prob_leave"] = vk[2]
